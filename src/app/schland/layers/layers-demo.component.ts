...
export class LeafletLayersDemoComponent {
    ...
    private readonly data = this.http.get(`http://${window.location.hostname}:8080/heat?responseDate=${new Date(new Date().getTime() - (3600 * 24 * 1000)).toISOString().substring(0, 10)}`).toPromise();
